#!/usr/bin/env python3
"""
Генератор переводов всех стран и регионов на 4 языка (RU, EN, ZH, ES)
"""
...
# Функция для получения перевода страны
...
# Функция для перевода регионов (упрощенная - используем транслитерацию или оригинал)
def translate_region(region_ru, lang):
    # Для большинства регионов используем оригинальное название
    # В реальном проекте здесь должен быть словарь переводов регионов
    return region_ru

def parse_locations_file():
    """Парсит файл locations.js и извлекает все страны с их данными"""
    with open('locations.js', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Находим секцию 'ru'
    ru_match = re.search(r"'ru':\s*\{([^}]+'en':)", content, re.DOTALL)
    if not ru_match:
        return None
    
    ru_section = ru_match.group(1)
    ...
    for line in ru_section.split('\n'):
        # Начало страны
        country_match = re.match(r"^\s+'([^']+)':\s*\{", line)
        if country_match:
            if current_country:
                countries[current_country] = {'regions': current_regions.copy()}
            current_country = country_match.group(1)
            current_regions = {}
            continue
        
        # Координаты страны
        coords_match = re.search(r"coords:\s*\[([^\]]+)\]", line)
        if coords_match and current_country and 'coords' not in countries.get(current_country, {}):
            coords = coords_match.group(1).strip()
            if current_country not in countries:
                countries[current_country] = {}
            countries[current_country]['coords'] = coords
        
        # Регион
        region_match = re.match(r"^\s+'([^']+)':\s*\{", line)
        if region_match and current_country:
            region_name = region_match.group(1)
            # Ищем влажность и координаты региона
            region_data = {'humidity': None, 'coords': None}
            # Читаем следующие строки для региона
            continue
        
        # Влажность региона
        humidity_match = re.search(r"humidity:\s*(\d+)", line)
        if humidity_match and current_country:
            # Это упрощенный парсинг - в реальности нужно более сложная логика
            pass
    
    # Сохраняем последнюю страну
    if current_country:
        countries[current_country] = {'regions': current_regions.copy()}
...
def generate_translations():
    """Генерирует полный файл locations.js с переводами на все языки"""
    print("Чтение исходного файла...")
    
    with open('locations.js', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Извлекаем русскую секцию полностью
    ru_match = re.search(r"'ru':\s*\{([^}]+'en':)", content, re.DOTALL)
    if not ru_match:
        print("Ошибка: не найдена секция 'ru'")
        return
    
    ru_content = ru_match.group(1)
    
    # Парсим страны из русской секции
    countries_ru = {}
    lines = ru_content.split('\n')
    ...
    while i < len(lines):
        ...
        if country_match:
            if current_country:
                current_country_data['regions'] = current_regions.copy()
                countries_ru[current_country] = current_country_data.copy()
            current_country = country_match.group(1)
            current_country_data = {}
            current_regions = {}
            i += 1
        ...
        # Координаты страны
        coords_match = re.search(r"coords:\s*\[([^\]]+)\]", line)
        if coords_match and current_country:
            current_country_data['coords'] = coords_match.group(1).strip()
            i += 1
            continue
        
        # Начало секции regions
        if 'regions:' in line and current_country:
            i += 1
            continue
        ...
        if region_match and current_country:
            if current_region:
                current_regions[current_region] = current_region_data.copy()
        ...
        # Влажность региона
        humidity_match = re.search(r"humidity:\s*(\d+)", line)
        if humidity_match and current_region:
            current_region_data['humidity'] = humidity_match.group(1)
            i += 1
            continue
        
        # Координаты региона
        region_coords_match = re.search(r"coords:\s*\[([^\]]+)\]", line)
        if region_coords_match and current_region:
            current_region_data['coords'] = region_coords_match.group(1).strip()
            i += 1
            continue
        
        i += 1
    
    # Сохраняем последний регион и страну
    if current_region:
        current_regions[current_region] = current_region_data.copy()
    if current_country:
        current_country_data['regions'] = current_regions.copy()
    ...
    print(f"Найдено стран: {len(countries_ru)}")
    
    # Генерируем переводы для всех языков
    languages = ['en', 'zh', 'es']
    translations = {'ru': countries_ru}
    ...
    for lang in languages:
        ...
        for country_ru, country_data in countries_ru.items():
            ...
            # Переводим регионы (упрощенно - используем оригинал)
            for region_ru, region_data in country_data['regions'].items():
                translations[lang][country_translated]['regions'][region_ru] = region_data.copy()
    
    # Генерируем новый файл
    print("Генерация нового файла locations.js...")
    ...
    for lang in ['ru', 'en', 'zh', 'es']:
        ...
        for country_name in sorted(countries.keys()):
            ...
            for region_name in sorted(country_data['regions'].keys()):
                ...
                humidity = region_data.get('humidity', 'null')
                coords = region_data.get('coords', 'null, null')
...
"""
    ...
    print("Готово! Файл locations.js обновлен с переводами на все 4 языка.")
