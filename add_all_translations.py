#!/usr/bin/env python3
"""
Добавляет переводы всех стран на 4 языка в locations.js
Работает напрямую с оригинальным файлом, сохраняя структуру
"""
...
country_translations = {
    ...
    'ОАЭ': {'en': 'United Arab Emirates', 'zh': '阿联酋', 'es': 'Emiratos Árabes Unidos'},
...
print("Чтение locations.js...")
with open('locations.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Находим границы секций
ru_start = content.find("'ru': {")
en_start = content.find("'en': {", ru_start)
end_pos = content.find("};", en_start)

if ru_start == -1 or en_start == -1:
    print("Ошибка: не найдены секции")
    exit(1)

# Извлекаем русскую секцию
ru_section = content[ru_start:en_start]

# Извлекаем английскую секцию (частичную)
en_section = content[en_start:end_pos]

# Парсим все страны из русской секции
print("Парсинг русской секции...")
countries_ru = {}

# Используем более точный парсинг построчно
lines = ru_section.split('\n')
current_country = None
current_coords = None
current_regions = {}
current_region = None
current_region_humidity = None
...
for line in lines:
    ...
    # Страна (8 пробелов, начинается с 'Название': {)
    if indent == 8:
        country_match = re.match(r"'([^']+)':\s*\{", stripped)
        if country_match:
            if current_country:
                countries_ru[current_country] = {
    ...
    # Координаты страны (12 пробелов, coords: [...])
    if indent == 12 and 'coords:' in line and current_country and not in_regions:
        coords_match = re.search(r"coords:\s*\[([^\]]+)\]", line)
        if coords_match:
            current_coords = coords_match.group(1).strip()
            continue
    
    # Начало regions (12 пробелов)
    if indent == 12 and 'regions:' in line and current_country:
        in_regions = True
        continue
    
    # Регион (16 пробелов)
    if indent == 16 and in_regions:
        region_match = re.match(r"'([^']+)':\s*\{", stripped)
        if region_match:
            if current_region and current_region_humidity is not None:
                current_regions[current_region] = {
                    'humidity': current_region_humidity,
    ...
    # Влажность региона (20 пробелов)
    if indent == 20 and 'humidity:' in line and current_region:
        humidity_match = re.search(r"humidity:\s*(\d+)", line)
        if humidity_match:
            current_region_humidity = int(humidity_match.group(1))
            continue
    
    # Координаты региона (20 пробелов)
    if indent == 20 and 'coords:' in line and current_region:
        region_coords_match = re.search(r"coords:\s*\[([^\]]+)\]", line)
        if region_coords_match:
            current_region_coords = region_coords_match.group(1).strip()
            continue
...
print(f"Извлечено стран: {len(countries_ru)}")
...
for lang in ['en', 'zh', 'es']:
    ...
    for country_ru, country_data in countries_ru.items():
        ...
        translations[lang][country_translated] = {
            ...
            'regions': country_data['regions'].copy()
        }
