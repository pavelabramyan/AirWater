#!/usr/bin/env python3
"""
Полный генератор переводов всех стран и регионов на 4 языка
"""

import re
import json

# Загружаем словарь переводов стран из отдельного файла или используем встроенный
country_translations = {
    ...
    # Добавляем все остальные страны из файла
...
def parse_js_object(content, start_pos=0):
    """Парсит JavaScript объект из строки"""
    pos = start_pos
    depth = 0
    in_string = False
    string_char = None
...
def extract_countries_from_ru_section(content):
    """Извлекает все страны из русской секции"""
    # Находим начало русской секции
    ru_start = content.find("'ru': {")
    if ru_start == -1:
        return None
    
    # Находим конец русской секции (начало английской)
    ...
    # Парсим страны используя регулярные выражения
    countries = {}
    
    # Паттерн для страны: 'Название': { coords: [...], regions: {...} }
    country_pattern = r"'([^']+)':\s*\{([^}]+coords:\s*\[([^\]]+)\][^}]+regions:\s*\{([^}]+)\})\}"
    
    matches = re.finditer(r"'([^']+)':\s*\{", ru_section)
    ...
    for match in matches:
        ...
        # Ищем coords
        coords_match = re.search(r"coords:\s*\[([^\]]+)\]", ru_section[start_pos:start_pos+500])
        if not coords_match:
            continue
        ...
        # Ищем regions
        regions_start = ru_section.find("regions:", start_pos)
        if regions_start == -1:
            continue
        
        regions = {}
        # Парсим регионы
        region_pattern = r"'([^']+)':\s*\{\s*humidity:\s*(\d+),\s*coords:\s*\[([^\]]+)\]\s*\}"
        region_matches = re.finditer(region_pattern, ru_section[regions_start:regions_start+5000])
        
        for reg_match in region_matches:
            region_name = reg_match.group(1)
            humidity = reg_match.group(2)
            region_coords = reg_match.group(3).strip()
            regions[region_name] = {
...
def generate_full_file():
    """Генерирует полный файл locations.js с переводами"""
    print("Чтение файла locations.js...")
    
    with open('locations.js', 'r', encoding='utf-8') as f:
        content = f.read()
    
    print("Извлечение стран из русской секции...")
    countries_ru = extract_countries_from_ru_section(content)
    
    if not countries_ru:
        print("Ошибка: не удалось извлечь страны")
        return
    
    print(f"Найдено стран: {len(countries_ru)}")
    
    # Генерируем переводы
    translations = {'ru': countries_ru, 'en': {}, 'zh': {}, 'es': {}}
    ...
    for lang in ['en', 'zh', 'es']:
        ...
        for country_ru, country_data in countries_ru.items():
            ...
            # Регионы пока оставляем на русском (можно добавить переводы позже)
...
"""
    ...
    print(f"Готово! Файл locations.js обновлен.")
